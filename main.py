...
def main(config):
    ...
    if config.MODEL.RESUME:
        ...
        acc1, acc5, loss = validate(config, data_loader_val, model, variance, noise, is_clip)
...
@torch.no_grad()
def validate(config, data_loader, model, variance, is_clip, noise):
    criterion = torch.nn.CrossEntropyLoss()
    model.eval()
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        print("idx: ",idx)
        if is_clip:
            images = torch.clip(images, 0.0, 1.0).cuda()
        if True:
            print("images before noise: ",images)
            print("max: ",torch.max(images))
            print("")
            print("randn before ",torch.randn(images.shape).cuda(),"\n")
            images += torch.randn(images.shape).cuda()*(variance**(.5))
            print("randn after ",torch.randn(images.shape).cuda()*(variance**(.5)),"\n")
            print("images after noise: ",images)
            print("max: ",torch.max(images))
            print("","\n")
